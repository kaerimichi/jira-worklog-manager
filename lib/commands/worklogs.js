...
function getCustomPeriod () {
  // TODO: return an array of dates (initially of the current week)
  return null
}
...
module.exports = async (options, { jira }) => {
  try {
    ...
    const customPeriod = options.week ? getCustomPeriod() : null
    const { data } = await jira.retrieveWorklogs(options.date || customPeriod)
