...
function getWorklogsToRegister (issue, date, worklogEntry, startTime, endTime) {
  const issueTotal = issue.split(',').length
  const multipleIssues = issueTotal > 1
  const issuesToRegister = multipleIssues
    ? issue.split(',').map(e => e.replace(' ', ''))
    : [issue]
  const duration = multipleIssues
    ? Math.floor((getIntervalAsSeconds(startTime, endTime) / 60) / issueTotal)
    : null

  return issuesToRegister.map(issue => {
    const finalDuration = worklogEntry.duration || duration

    return {
      message: finalDuration
        ? `${date}: ${finalDuration} minutes on issue/alias ${issue}`
        : `${date}: from ${startTime} to ${endTime} on issue/alias ${issue}`,
      date,
      issue,
